...
class VentaController extends Controller
{
    ...
    public function index(Request $request){
        ...
        public function create(){
             ...
             ->join('detalle_compras','prod.id','=','detalle_compras.idequipo')
             ->select(DB::raw('CONCAT(prod.activo_fijo," ",prod.nombre) AS equipo'),'prod.id','prod.stock','prod.precio_venta')
             ->where('prod.condicion','=','1')
             ->where('prod.stock','>','0')
             ->groupBy('equipo','prod.id','prod.stock','prod.precio_venta')
             ->get(); 
 
 
             return view('venta.create',["clientes"=>$clientes,"equipos"=>$equipos]);
         ...
         public function show($id){
             ...
             /*mostrar detalles*/
             $detalles = DetalleVenta::join('equipos','detalle_ventas.idequipo','=','equipos.id')
             ->select('detalle_ventas.cantidad','detalle_ventas.precio','detalle_ventas.descuento','equipos.nombre as equipo')
